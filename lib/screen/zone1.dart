...
class Zone1Page extends StatefulWidget {
  ...
  Zone1Page({required this.humidity});
...
class _Zone1PageState extends State<Zone1Page> {
  ...
  void initState() {
    ...
    _loadAirHumidityFromFirebase();
  }

  void triggerFirebaseButton() async {
    final ref = FirebaseDatabase.instance.ref('Fib_Buttons/Fib_Button1');

    await ref.set(64); // ส่งค่า 1
  }

  void _loadFibTimestartZ1() async {
    DatabaseReference ref = FirebaseDatabase.instance.ref('FibTimestartZ1');
  ...
  void _loadFibTimestopZ1() async {
    DatabaseReference ref = FirebaseDatabase.instance.ref('FibTimestopZ1');
  ...
  void _loadAirHumidityFromFirebase() {
    DatabaseReference ref = FirebaseDatabase.instance.ref('Hum');

    ref.onValue.listen((event) {
      final data = event.snapshot.value;
      if (data != null && data is num) {
        setState(() {
          airHumidity = data.toDouble();
        });
      }
    });
  }
  ...
  void _listenModeStatusFromFirebase() {
    DatabaseReference ref =
        FirebaseDatabase.instance.ref('Fib_Buttons/AuTo_Z1');
  ...
  void _loadCurrentHumidityFromFirebase() async {
    DatabaseReference ref =
        FirebaseDatabase.instance.ref('Soil_As'); // ใช้ path ที่ถูกต้อง
  ...
  void _loadTargetHumidityFromFirebase() {
    DatabaseReference ref = FirebaseDatabase.instance.ref('FibSoilZ1');
  ...
  void _saveMode(bool manualMode) async {
    ...
    prefs.setBool('isManualModeZone1', manualMode);
  }
  ...
  void _loadMode() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isManualMode = prefs.getBool('isManualModeZone1') ?? true;
    });
  }

  void _saveTargetHumidity(double humidity) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setDouble('targetHumidityZone1', humidity);
    prefs.setString('setHumidityMessageZone1',
        'ค่าความชื้นที่ตั้งไว้: ${humidity.toStringAsFixed(0)}%');

    FirebaseDatabase.instance.ref('FibSoilZ1').set(humidity);
  ...
  void _loadTargetHumidity() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    double? savedHumidity = prefs.getDouble('targetHumidityZone1');
    String? savedMessage = prefs.getString('setHumidityMessageZone1');
  ...
  void _saveWaterStatus(bool status) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setBool('isWaterOnZone1', status);
  }

  void _loadWaterStatus() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    bool? savedWaterStatus = prefs.getBool('isWaterOnZone1');
  ...
  void _toggleMode(bool manualMode) {
    ...
    FirebaseDatabase.instance.ref('Fib_Buttons/AuTo_Z1').set(modeValue);
    ...
    // ✅ ปิดน้ำทันทีเมื่อเปลี่ยนจาก Manual → Aut2
    if (!manualMode) {
  ...
  void _saveFibTimestartZ1() async {
    String time = _openWaterController.text;
    if (time.contains(":")) {
      FirebaseDatabase.instance.ref('FibTimestartZ1').set(time);
    }
  }

  void _saveFibTimestopZ1() async {
    String time = _closeWaterController.text;
    if (time.contains(":")) {
      FirebaseDatabase.instance.ref('FibTimestopZ1').set(time);
    }
  }
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    const Text('ค่าความชื้น (%)',
                        style: TextStyle(
                            fontSize: 20, fontWeight: FontWeight.bold),
                        textAlign: TextAlign.center),
                    const SizedBox(height: 24),

                    // วงกลมแสดงความชื้นอากาศ
                    _buildHumidityCircle(label: 'อากาศ', value: airHumidity),

                    const SizedBox(height: 32),

                    // วงกลมแสดงความชื้นดิน
